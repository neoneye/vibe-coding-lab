...
- Original stage 5 (Lamzouri stability slack): **conducted** on 2026-09-08 (`LAMZOURI_STABILITY.md`, `lamzouri_slack.py`): the proof's six discarded quantities reproduce the slack to 1e−13 on every family; on the zeros the slack is exactly the off-diagonal pair mass; the simple-simple pair energy is a valid correction term (`Δ ≥ D1` on all tests, equality on all-simple sets) but carries no new information, and the all-pairs energy is refuted by off-line pairs (counterexamples kept). Closed by the plan's own stopping rule.
...
**A soundness fault in the checker, found by a negative control.** The 5000-sample run reported 1407 cases where the natural and centred enclosures of one quantity were *disjoint* — impossible if both are enclosures. All were point boxes, the two balls differing by ~1e−20 with radii ~1e−60. Cause: the natural path converted pair-distance balls to Python floats (`float(d.lower())`) before the exact kernel-range lookups, and `span` dropped the midpoint's own radius when its inputs were balls, and the centred value form evaluated the piecewise-linear parts at a rounded centre. Every such conversion is now outward (`math.nextafter`), `span` carries the midpoint radius, and centre evaluations are padded; disjoint pairs are now a failed check that dumps the boxes. After the repair: 0 conflicts on the pair tape, on the small `sharp` domain (every obligation, depth 4: 2013/3869 leaves, 382/988 collapses confirmed, 0 refuted) and on the full-tape 5000 sample.


**The resolution problem was the same interval.** With the pair distance enclosed exactly — the sum of the gap intervals between the two points, `[Σ_{k∈[i,j)} lo_k, Σ hi_k]`, which is what the sweep itself uses — instead of the difference of prefix sums, the checker resolves everything it is given: the pair transcript is 220/220 on both counts, and on the small `sharp` domain every one of the 3869 leaves and 988 collapses is confirmed at depth 0, nothing refuted, no disjoint pairs. "Beyond this checker's resolution" had been the loose interval, not a limit of Arb or of the centred forms. A complete arithmetic verification of the full `sharp` tape (48 million obligations at ~2.3 ms each) was launched in eight root-range shards; its outcome is recorded below when the shards finish.

**A second, older soundness fault, found by the same control at larger scale.** A 50 000+50 000 refined sample of the full `sharp` tape (46 322 and 47 973 confirmed, 0 refuted) still reported 20 disjoint value enclosures, all on wide boxes at the origin faces (widths 0.4–0.9, some coordinates from 0), and direct 40-digit evaluation inside them showed the *natural* enclosure wrong by a factor of ten or more (e.g. `[±0.089]` against sampled values in `[0.55, 2.15]`), the centred one right. Cause: the natural extension encloses a pair distance `y_j − y_i` by the dependency-blind interval `[Σlo − Σhi, Σhi − Σlo]`, which reaches below zero on such boxes, while the exact-range tables of `kernel_pieces_arb.py` hold only the *positive* breakpoints of `w` and `w′` — so an interval straddling zero missed `w`'s maximum at 0, and one reaching below `−z₁ ≈ −1.057` would miss its minimum there, which is the direction that could confirm a false leaf. The true distance is a sum of gaps and is nonnegative, so the interval is now clamped at zero before every lookup (both in the natural path and in the centred gradient's second-derivative term). This fault predates the investigation (it is in the checker as committed for the pair tape). After the clamp the pair transcript (39/220, 67/220) and the small `sharp` domain reproduce exactly, so none of those confirmations rested on it; the 20 witness boxes are kept in `sweep_proof_sharp_full.sample50000.results.conflicts.json`, and the full-tape samples (sharp 5000 and 50 000, pair full cube) were re-run with the repaired checker — results in the transcripts of the same names. The centred form for the gradient (mean-value theorem with `weight_dd`, falling back to the natural range at the kernel's removable singularity) raised collapse confirmations at depth 4 from 299 to 382 of 988.


## Deliverables, against the follow-up plan's list

1. **Corrected oracle and claim ledger** — `labf_oracle.py` distinguishes exact-decimal from binary64 input, converts before subtracting, recomputes constants per precision; `CLAIM_LEDGER.md` names actual verification strengths and sample domains.
2. **Pinned upstream, patch, build commands, statements, axiom logs** — `lean/README.md`, `lean/window-sum.patch` (applies cleanly to a fresh clone at `47d0241`, checked), `lean/Signed.lean`, `lean/SharpPotential.lean`, `lean/build-axioms.log`: the conservative variant (`n_point_bound_of_windowSum`), the signed variant (`n_point_bound_signed`) and the concrete `sharp` instance (`sharp_chain_bound_signed_concrete`), all standard axioms.
3. **Runnable projection calculation** — `projection.py` (exact rationals for `c, B, W, m`; both scores; integer cap checks), `endpoint_oscillation.py` (exact endpoint bounds, pair certificate by DP), `signed_endpoint_model.py`.
4. **The selected certificate and its verification coverage** — `sharp`: rigorous transcript restored (67 608 431 boxes, checksum `37308f214a50bb25`); double-precision proof tape (74 694 256 nodes) structurally verified in full; Arb arithmetic on 5000+5000 refined samples, 4657 and 4798 confirmed, 0 refuted, 0 disjoint enclosures; a larger sample running. The remaining obligation is a *complete* arithmetic verification of the tape, which at ~2–5 ms per node with subdivision is of order days, not hours; the checker's collapse resolution is its limiting factor.
5. **Which statement is proved and which hypotheses remain** — proved: everything from the certificate to the proportion of zeros, for Mathlib's `riemannZeta`, with the shipped `sharp` potential and its bounds inside Lean. Remaining: `hcob`, the coboundary inequality on every window of every sorted 258-point list (the sweep's claim, on this page's arithmetic, sample-checked in Arb).

Not done: the eight-point route with amplitude control (step 6 of the first plan; its payoff is `+2·10⁻⁵` conditional on a certificate that does not exist and a seven-dimensional sweep), and the pressure re-optimisation at integer `p` (the ceilings move by `3·10⁻⁸`; no certificate exists at any `p ≠ 3000`).
