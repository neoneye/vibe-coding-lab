...
Both are parameters of `n_point_bound` (`p : ℕ`, so integer pressure denominators only — a real optimum such as 3370.45 needs integer neighbours or an extension; at the integer neighbours the note's own ceiling projection `tiling_pressure.projectionAt` gives 0.6731150210 at `p = 3370` (period-two branch, 275 windows) and 0.6731149900 at `p = 3371` (period-three), against 0.6731150213 at the real crossing, so integrality costs 3·10⁻⁸ — all of these ceilings, not certified floors), with side conditions `2 ≤ n ≤ m`, `0 < p`, `0 < c`, `c(m−(n−1)) ≤ 1`, and the certificate at the same `(n, p)`. This answers the two questions the research note put to "whoever has the manuscript": the `1/3000` is a choice, not a derivation, and general `n` is allowed. The block-size scan and the pressure optimisation of the note are therefore mathematics about a well-posed parameter of the theorem — still subject to the missing lemma below, since they were run on chain floors.
...
**The signed route (follow-up plan, step 3) — proved.** Keeping `Δ_s = Φ(state_{s+W}) − Φ(state_s)` signed through S13 and summing over every consecutive full block in S15 before bounding, `Σ_s Δ_s` telescopes to at most `W·B` independent of the configuration length, and the S13 clipping survives under `c·W + B ≤ 1`. This is now Lean: `Signed.lean` in the branch (`investigation/lean/Signed.lean`), seven declarations ending in `n_point_bound_signed` and the instance `sharp_chain_bound_signed`, all with standard axioms. `sharp` meets the cap at `W = 252` (`cW + B = 0.99862`), so its conditional projection is `Phi_n 7 (3956/10⁶) 258 3000 = 0.6731093501` with no endpoint penalty; the pinned pair certificate meets it only at `W = 250` (0.6731098447). The finite model `investigation/signed_endpoint_model.py` remains as the exact-rational check of the same bookkeeping. `SharpPotential.lean` then defines the shipped `sharp` state potential in Lean (clamped piecewise-linear interpolation of the 51 exact binary64 values, `plClamp`) and proves its bounds and oscillation, so `sharp_chain_bound_signed_concrete` has the sweep's coboundary inequality as its **only** hypothesis.

Remaining numerical premise for either score: the `sharp` chain floor `0.003956` on `[0,16]⁶` with the tail lemma. Its rigorous transcript had been **absent** from `tiling_interval.results.json`; it was re-run on 2026-09-07 (`complete`, 67 608 431 boxes, checksum `37308f214a50bb25`, 4764.8 s, current hashes) and is now the table's transcripted rigorous rung. Independent of that run, its double-precision proof tape (74 694 256 nodes) is structurally verified in full and Arb-checked on a refined sample with nothing refuted (`REPORT.md`).
